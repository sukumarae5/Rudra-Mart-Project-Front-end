...
const fetchWishlistApi = async () => {
  ...
  const response = await fetch("http://192.168.1.10:8081/api/wishlist/all", {
    method: "GET",
    headers: {
...
const addToWishlistApi = async (productid) => {
  ...
  const response = await fetch("http://192.168.1.10:8081/api/wishlist/additem", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${userToken}`,
...
const deleteItemApi = async (wishlistid) => {
  ...
  const response = await fetch(`http://192.168.1.10:8081/api/wishlist/${wishlistid}`, {
    method: "DELETE",
