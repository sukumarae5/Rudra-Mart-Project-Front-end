...
const fetchCartDataApi = async () => {
  ...
  const response = await fetch("http://192.168.1.10:8081/api/cart/my-cart", {
    method: "GET",
    headers: {
...
const removeCartItemApi = async (cartItemId) => {
  ...
  const response = await fetch(`http://192.168.1.10:8081/api/cart/${cartItemId}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${userToken}`,
    },
...
const updateCartItemQuantityApi = async ({cartItemId, quantity }) => {
  ...
  const response = await fetch(`http://192.168.1.10:8081/api/cart/update/${cartItemId}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${userToken}`,    
